...
fgets(poubelle, sizeof(ligne), ptr_sauvegarde); // Pour sauter la première ligne qui est tout simplement le détail de ce que représente chaque colonnes
    ...
    ptr_fichier = fopen(FichierEcole, "r"); // ouverture du fichier en mode lecture
    ...
    for(i=0; i<nb; i++) // on parcourt le tableau de classe
    {
...
void Ecrire_Fichier_Classe(const char *FichierEcole2, Ecole_t E)
